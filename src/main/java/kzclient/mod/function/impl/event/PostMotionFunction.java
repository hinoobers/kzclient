...
public class PostMotionFunction extends ModFunction {
    ...
    public void draw(int mouseX, int mouseY, int x, int y) {
        ...
        font.drawString("Post Motion Event", x + 5, y + 15, 0xFFFFFFFF);
        int i = 0;
    ...
    public void fire(ObjectRegistry registry) {
        System.out.println("Post Motion Event");
        this.outputs.forEach(out -> {
            out.getLines().forEach(e -> e.input.parentFunction.fire(null));
        });
    }
    ...
    public ModFunction _copy() {
        PostMotionFunction function = new PostMotionFunction();
