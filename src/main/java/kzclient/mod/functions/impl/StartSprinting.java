...
public class StartSprinting extends ModFunction {

    public StartSprinting() {
    ...
    public void draw(GuiScreen screen, int x, int y, int mouseX, int mouseY) {
        ...
        screen.drawCenteredString(fontRenderer, "START SPRINTING", x + this.width / 2, y + this.height / 2 - 10, Color.white.getRGB());
    ...
    @Override
    public StartSprinting _copy() {
        StartSprinting copy = new StartSprinting();
        copy.x = this.x;
        copy.y = this.y;
